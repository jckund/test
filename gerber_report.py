...
LOCATIONS_PATH = os.path.join("data", "gerber", "locations.json")
CHANGES_PATH = os.path.join("data", "gerber", "changes.jsonl")
...
def fmt_line_html(loc):
    ...
    cid = html.escape(loc.get("storeId") or "")
...
def build_email(added, removed, total, when):
    ...
    if quiet:
        subject = f"Gerber locations: no changes this week ({total:,} tracked)"
    else:
        subject = f"Gerber locations: +{len(added)} new, −{len(removed)} removed (week of {week})"
...
<h1 style="font-size:20px;margin:0 0 4px">Gerber Collision & Glass — weekly location changes</h1>
...
Source: <a href="https://www.gerbercollision.com/locations" style="color:#999">gerbercollision.com/locations</a>
&middot; generated {when.strftime('%Y-%m-%d %H:%M UTC')} by the gerber_locations GitHub Action.
</p></div>"""
